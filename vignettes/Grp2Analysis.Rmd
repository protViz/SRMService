...
The following analysis compars protein signal intensities recorded in two groups of samples (Tables \@ref(tab:samples) and \@ref(tab:annotation)) by computing the fold change $log2(condition/reference)$, and by testing if this fold change (also later called effect size) is different from zero. Which of the two groups is used as reference is shown in the table \@ref(tab:groupingvars).


The protein identification and quantification was performed using the _MaxQuant_ software and _Andromeda_ search engine [@Cox2008, Cox2011].  Based on protein intensities reported by MaxQuant in `proteinGroups.txt` file, we run a _moderated t-test_ [@Smyth2004] for all proteins quantified with at least `r grp2$nrPeptides` the R package _limma_ [@Ritchie2015a]. 
...
knitr::kable(x, caption="The reference group is in the denominator of the foldchnages.")
```
...
```{r missingValuesPerProtein, fig.cap="# of missing values per protein", fig.height=3,fig.width=5}
barplot(table(missing), xlab="number of missing values", ylab="number of proteins")
...
```{r distributionRaw, fig.width=8, fig.height=5, fig.cap="Violin plots for quantifyable proteins (log2 transformed)"}
...
```{r normalized,  fig.width=8,fig.height=5,dpi=300, fig.cap="Violin plots of normalized protein intensity values (z-score)"}
...
The Figure \@ref(fig:SDViolin) left panel, show the coefficient of variations for all proteins in each condition and overall computed on not normalized data. To observe differences between conditions the variation within a condition should ideally be smaller than within all conditions.
...
```{r SDViolin, fig.height=3, fig.width=7, fig.cap="Left panel - Distribution of protein CV within condition and overall, Right panel - Distribution of protein standard deviation (after sample normalization and scaling) within conditions and overall"}
...
x <- data.frame(table(abs(tmp$effectSize) > grp2$qfoldchange & tmp$q.mod < grp2$qvalue))
if(length(x$Var1) == 2){
...
## List of Columns in the output table

```{r outputdatacolumns}
knitr::kable(data.frame(columns=colnames(grp2$getResultTableWithPseudo())))
```
...
You can reference this analysis by providing the following URL: www.github.com/protViz/SRMService 

This report was generated using the package [SRMService](http://github.com/protViz/SRMService) and [quantable](http://github.com/protViz/quantable). The q-values and p-values were computed using the bioconductor package and [limma](https://bioconductor.org/packages/release/bioc/html/limma.html).
