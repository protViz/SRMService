...
If the groups are different we would expect more small p-values in Figure \@ref(fig:densityOFFoldChanges) than by chance (blue horizontal line). If there are only as many or less small p-values as by chance than no significant false discovery rate controlled calls (Q Values) will be made in Figure \@ref(fig:volcanoplot). 
...
If you are going to use results produced by the scripts please do cite the SRMSerivce R package by W.E. Wolski, J. Grossmann, C. Panse and providing the following URL: www.github.com/protViz/SRMService 
