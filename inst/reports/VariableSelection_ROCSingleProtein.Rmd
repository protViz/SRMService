...
p <-ggplot(results, aes(log2FoldChange, -log10(pvalue))) + geom_point(mapping=aes(x=log2FoldChange, y=-log10(pvalue), shape=isGLM, col=isGLM), size=2)
...
filtres <- subset(results, (labels %in%  addPlot) |  (pvalue<pthresh & abs(log2FoldChange)>log2FCThresh) )
...
trash2 <- data.frame(paste(results$labels[results$isGLM], "_HUMAN", sep=""), color="#ff0000")
write.tab(trash2, file="string/GLMimportant.txt")
