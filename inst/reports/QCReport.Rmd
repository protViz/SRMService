---
title: "FGCZ MaxQuant Protein Groups QC-Report Analysis: `r grp2$projectName` \n Statistics for a Quantitative Protein Matrix"
...
plot(cumsum(table(missing))/length(missing)*100, ylab="% of proteins", xlab="# of missing values per protein")
...
mar <-par()$mar
par(mar = c(10,3,3,3))
barplot(quantable::colNAs(grp2$proteinIntensity)/nrow(grp2$proteinIntensity)*100, las=2, ylab="percent missing")
par(mar= mar)
```
...
simpleheatmap(
  grp2$getNormalized()$data[grp2$getNrNAs() < ncol(grp2$getNormalized()$data)/2,],
  margins=c(10,3) 
)
