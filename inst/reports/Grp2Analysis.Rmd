...
In Figure \@ref(fig:scaling) the ratios of the average sample intensity versus the mean average intensity of all samples is shown. It is critical if a samples average deviates more than 5 times from mean of all samples.

```{r  scaling,dpi=300, fig.height=6, fig.cap="Average intensity in sample vs average intensity in all samples."}
par(mar=c(3,10,3,3))
bb <- grp2$getNormalized()$medians

barplot(sort(abs(bb))  - mean(bb) ,horiz=F,las=2, main="median", cex.names = 0.6, ylab="log2(median(I)) - mean(log2(median(I))")
abline(h=c(-5,5),col=2)
x<-seq(-2,2,by=.5)
...
In the following analysis we assume that most of the proteins are not regulated (fold change is equal zero). p-values and Q-values are a measure of how likely it is to observe the data given this assumption ($H_0$). Figure \@ref{fig:densityOFFoldChanges} shows the distribution of fold changes.
...
plot(density(res.eb$effectSize))
...
If the groups are different we would expect more small p-values in Figure \@ref{fig:histOfpValues} than by chance (blue horizontal line). If there are only as many or less small p-values as by chance than no significant false discovery rate controlled calls (Q Value) will be made in Figure \@ref{fig:volcanoplot}. 
