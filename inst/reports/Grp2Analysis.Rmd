...
```{r include=FALSE}
fixcols <- function(resRel, name = "qvalue", top = 20){
  resRel<-do.call("rbind", resRel)
  rownames(resRel) <- NULL
  resRel <- data.frame(resRel,tmp= 2^resRel$foldchange)
  colnames(resRel) <- c("regulation", "Accession", "log2(T/N)", name, "foldchange (T/N)")
...
resStr <-quantable::volcanoplot(res.eb$logFC, res.eb$q.mod, foldchangethresh = grp2$qfoldchange , pthresh = grp2$qvalue ,labels=rownames(res.eb),xlab = paste("log2(",unique(grp2$annotation$Condition)[2],"/",unique(grp2$annotation$Condition)[1],")"))
...
```{r results="markup"}
if(!is.null(resStr)){
  resStr<-fixcols(resStr,name="qvalue")
...
res.eb <-grp2$getPValues()
resStr <- quantable::volcanoplot(res.eb$logFC, res.eb$p.mod, foldchangethresh = grp2$pfoldchange , pthresh = grp2$pvalue ,labels=rownames(res.eb), xlab = paste("log2(",unique(grp2$annotation$Condition)[2],"/",unique(grp2$annotation$Condition)[1],")"))
```
...
resRel <- quantable::volcanoplot(res.eb$logFC, res.eb$p.ord, foldchangethresh = grp2$pfoldchange , pthresh = grp2$pvalue ,labels=rownames(res.eb),xlab = paste("log2(",unique(grp2$annotation$Condition)[2],"/",unique(grp2$annotation$Condition)[1],")"))
...
This document was generated using Rmarkdown and processes text files which are generated either with a label-free quantitation software such as  MaxQuant. In anycase the obtained results should be validated orthogonally as well (e.g. with Western blots).
