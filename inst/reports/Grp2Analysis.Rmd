...
col <- c("red","blue")
simpleheatmap(cor(grp2$getNormalized()$data , use="pairwise.complete.obs", method="spearman")^2,palette = getGreensScale(21),ColSideColors = col[as.factor(grp2$annotation$Condition)], margins = c(10,3))
...
simpleheatmap(grp2$getNormalized()$data[grp2$getNrNAs() < ncol(grp2$getNormalized()$data)/2,],ColSideColors = col[as.factor(grp2$annotation$Condition)], margins=c(10,3) )
...
two group analysis using bioconductor limma. Significant calls are made with q Value smaller 0.01 (false discovery rate, computed with bioconductor package qvalue). The list is also written to a text file with group means,
pValue, FDR (qValue), FoldChange. Additionally we output graphically the Volcano plot.
...
resStr <-quantable::volcanoplot(res.eb$logFC, res.eb$q.mod, foldchangethresh = 0.1 , pthresh = 0.01 ,labels=rownames(res.eb))

```

```{r}
resStr<-fixcols(resStr)
knitr::kable(resStr, caption = "Stringent cutoff (q < 0.01) regulated proteins.")
```


## Relaxed Threshold

ere we show for the normalized matrices the result of a
two group analysis using bioconductor limma. Significant calls are made with qValue smaller 0.05 (false discovery rate, computed with bioconductor package qvalue). We output first the length of the list (the list is also written to a text file with group means,
pValue, FDR, FoldChange). Additionally we output graphically the Volcano plot



```{r fig.width=10, fig.height=8, dpi=300, fig.cap="VolcanoPlot for normalized and imputed matrix (asinh) with relaxed thresholds."}
res.eb <-grp2$getPValues()
resRel <- quantable::volcanoplot(res.eb$logFC, res.eb$q.mod, foldchangethresh = 0.1 , pthresh = 0.05 ,labels=rownames(res.eb))
...
```{r}
resRel <- fixcols(resRel)
knitr::kable(resRel, caption = "Relaxed cutoff (q < 0.05) regulated proteins.")
...
This document was generated using Rmarkdown and processes text files which are generated either with a label-free quantitation software such as ProgenesisLCMS or MaxQuant. In anycase the obtained results should be validated orthogonally as well (e.g. with Western blots).
