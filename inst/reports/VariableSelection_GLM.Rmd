...
iprotV <- data.frame(p2grp[,3:ncol(p2grp)], Condition = as.numeric(grepl( Condition2Compare, p2grp$Condition)))


Variables <- t(combn(as.character(unique(protData$data$Protein)) , nvariables , FUN = NULL, simplify = TRUE ))
colnames(Variables) <- paste("v",1:ncol(Variables),sep="")

library(plyr)


modelData <- alply(Variables, 1, function(x, iprotV){
    tmp <- data.frame(Condition = as.factor(iprotV$Condition), iprotV[,x])
    tmp
  }, iprotV
)
...
makeAUC <- function(x){
  ...
  pROC::auc(controls = x[iprotV$Condition == 0 ] , cases = x[iprotV$Condition == 1])
}

resAUC <- rep(NA, length(resModels))
for(i in 1:length(resModels)){
  resAUC[i] <- makeAUC(resModels[[i]])
}
...
The total number of models is `r choose(length(unique(protData$data$Protein)), nvariables)`.
...
```{r fig.cap="Left panel: density of predictions values black - category 1; red - category 2 , Right panel : ROC curve"}
...
quantable::makeROCplot(x[iprotV$Condition == 0 ] , x[iprotV$Condition == 1])

```

```{r, fig.cap="normalized intensities of the best predictors"}
...
```{r, fig.cap="Significance of variables", fig.width=6, fig.height=6}
coefThreshold <- 0.5
...
# Analyse best pairs in top 10% models

```{r}
top10pcModels <- resultsCoeff[1:(nrow(resultsCoeff)/10),]

#x <- with(top10pcModels,unique(c(var.v1, var.v2, var.v3)))

res <- plyr::ddply(top10pcModels,
                   .(mID) ,
                   function(x){y <- t(combn(x[1:3],2)); y <- data.frame(a = unlist(y[,1]), b=unlist(y[,2]));y } )

x <- dplyr::left_join(res, top10pcModels)
```


```{r}
xxAUC <- x %>% group_by(a,b) %>% summarise(mean(AUC))
xxLength <- x %>% group_by(a,b) %>% summarise(length(AUC))
xxmin <- x %>% group_by(a,b) %>% summarise(min(AUC))
xxmax <- x %>% group_by(a,b) %>% summarise(max(AUC))

rrA <- inner_join(inner_join(inner_join(xxAUC, xxLength),xxmin),xxmax)
xxB <- arrange(rrA,desc(`length(AUC)`),desc(`mean(AUC)`))
knitr::kable(head(xxB))
