...
```{r make_summarytableBestPairs}
xxAUC <- x %>% group_by(a,b) %>% dplyr::summarise(mean(AUC))
xxLength <- x %>% group_by(a,b) %>% dplyr::summarise(length(AUC))
xxmin <- x %>% group_by(a,b) %>% dplyr::summarise(min(AUC))
xxmax <- x %>% group_by(a,b) %>% dplyr::summarise(max(AUC))
