...
* Summaries for NAs.
    ...
    * nr of NA's per precursor, peptide,
    - based on mice
        - `lcmsqc_cc` select complete cases and `lcmsqc_cci` complete case indicator
        - `lcmsqc_ic`	Select incomplete cases
        - `lcmsqc_ici`	Incomplete case indicator
...
* MA plots on various levels
   * Log2 fold change vs intensity for precursors, aggregated peptides, and proteins.
    `lcmsqc_gg_ma` see also (ma on ggplot2)[https://gist.github.com/slavailn/172302c273b737b4219516c93173542c]
...
- use tidyverse (or pandas)
- implement functions to switch between long to wide `to_wide` - pivot.
   - functions to switch from wide tolong `tolong` - difficult since wide cant cary the same amount of information?
   - the wide representation needs to store the more of information in an additional attribute?
- Use an additional object which describes which columns means what?
    - additional attributes e.g. `subjectComponents`
...
Do we also need export all intermediate results.
