...
\title{FGCZ p\Sexpr{projectID} order \Sexpr{orderID} \\ Order: \Sexpr{orderID}: QC analysis for sample preparation and LC-MS}
\begin{document}
...
# #qc_summary_table[,2] <- colorColumnIf(qc_summary_table[,2],3)
# #print(xtable(qc_summary_table, caption="Quality Control Summary", sanitize.text.function = function(x){x}), include.rownames=FALSE)
print(xtable(qc_summary_table, caption="Quality Control Summary, (1) Fold change threshold: 1.5, pValue threshold: 0.05 this is the percentage of false positives. As this is a QC analysis which consists of biochemical replicas where we do not expect to see real changes. (2) single hit proteins are proteins identified with only one peptide. This percentage can vary extensively and is largely sample dependent. Since we are going to quantify proteins with at least 2 peptides this shows the percentage one may loose for quantitation."), include.rownames=FALSE)
@
\vspace{1cm}
...
print(xtable(topTable, caption="Overview on the assingment rates for each file"), size="\\small")
...
plotDensity(qc_intensities, xlim=c(0,maxY), col=colors, lwd=1, main="Distribution of untreated Signals (no transformation)", xlab="Quantitative value (x-axis is truncated)")
...
plotDensity(asinh(qc_normalized_intensities), col=colors, lwd=1, main="Signal Distributions (imputed and normalized)", xlab="asinh(Quantvalue)")
legend("topright", samples, text.col=colors)
@
