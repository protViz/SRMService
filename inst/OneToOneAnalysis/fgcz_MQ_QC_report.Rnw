...
\caption{The {\emph scatterplot matrix} roughly determines if you have a
linear correlation with the logarithmically transformed signal between
multiple samples. The lower panels display the correlation between the
