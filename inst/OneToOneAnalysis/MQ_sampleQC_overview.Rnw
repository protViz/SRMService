...
\title{\Sexpr{projectNumber} - Label free quantification analysis: \\ \Sexpr{OID}: QC analysis for sample preparation and LC-MS}
%\title{FGCZ Two-Group Analysis for: \Sexpr{projectNumber}\\
...
#max % on var mods
#histogram(~ msms_d$Modifications|msms_d$Raw.file, main="VarMods per RawFile",  scales = list(x = list(rot = 90, cex=0.8)), par.strip.text = list(cex = 0.8), xlab="Variable modifications")
...
myFileTable <- as.matrix(cbind(as.character(files), betterNames))
colnames(myFileTable) <- c("original RawFileNames", "betterNames")
...
histogram(~ evi_d$m.z|evi_d$Raw.file, par.strip.text = list(cex = 0.8), xlab="precursors (m/z)")
@
...
\begin{figure}
<<echo=FALSE,fig=TRUE,width=12, height=8>>=
histogram(~ evi_d$Mass.Error..ppm. |evi_d$Raw.file, nbins=50, main="Distribution of recalibrated mass error (ppm)", par.strip.text = list(cex = 0.8), xlab="Recalibrated mass error (ppm)")
@
...
% histogram(~ msms_d$Length|msms_d$Raw.file, main="Length Distribution of identified peptides", par.strip.text = list(cex = 0.8), xlab="Peptide Length")
% @
...
N <- length(table(msms_d$Raw.file))
histogram(~ msms_d$Missed.cleavages|msms_d$Raw.file,main="Distribution of missed cleavages", par.strip.text = list(cex = 0.8), xlab="Number of missed cleavages")
@
...
histogram(~ msms_d$Modifications|msms_d$Raw.file, ylim=c(0,30), main="VarMods per RawFile",  scales = list(x = list(rot = 90, cex=0.8)), par.strip.text = list(cex = 0.8), xlab="Variable modifications")
@
...
#Charge
histogram(~ msms_d$Charge|msms_d$Raw.file, main="Distribution of Charges for identified peptides", par.strip.text = list(cex = 0.8), xlab="Charge of identified peptides")
@
...
heatmap.2(asinh(as.matrix(quantDat)), dendrogram = "none", trace = "none", labRow="",margins=c(15,2), main="Heatmap of quantitative value in all experiments", ylab="proteins")
@
...
maxY <- max(quantDat)/5
plotDensity(quantDat, xlim=c(0,maxY), col=colors, lwd=1, main="Histogram of untreated SignalDistributions (no transformation)", xlab="Quantitative value (x-axis is truncated)")
...
par(mfrow=c(1,1))
par(mar=c(4,10,4,1))
barplot(scaleF$V2, horiz=TRUE, names.arg=scaleF$V1, cex.names = 1, las=1, main="Applied scaling Factors (normalization)")
@
...
heatmap.2(as.matrix(cor(asinh(n_i_dat))),margin=c(15,10),trace="none", main="Sample Correlation")
...
<<echo=FALSE, fig=TRUE,  width=6, height=8>>=
mat <- n_i_dat
TotList <- SRMService:::Do2grpTtestOnMatrixAndBHcorrReturnAllInternalTrafo(ProtQuantMatrix=mat, bool_TrafoHere=TRUE)
