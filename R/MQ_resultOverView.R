...
#' render_MQ_resultOverview
#'
#' @param scratch
#' @param outputPDF
#'
#' @return
#' @export render_MQ_resultOverview
#'
#' @examples
#'
#' SRMService:::render_MQ_resultOverview(scratch='~/Downloads/WU153423')
#'
render_MQ_resultOverview <- function(scratch = '/tmp/',
                                     outputPDF = '~/Downloads/MQ_resultOverview.pdf',
                                     reportRmd = system.file("reports", "QCReport.Rmd", package = "SRMService")
                                     ){
  tempReport <- file.path(tempdir(), "MQ_resultOverview.Rmd")



  file.copy(reportRmd, tempReport, overwrite = TRUE)

  # global <<-
  compute.config <<- list(scratch=scratch)


  files <- list.files(scratch)
  yaml.filename <- files[grep(pattern="config.*yaml", x=files)]
  yaml.config <- yaml::yaml.load_file(paste(compute.config$scratch, yaml.filename, sep='/'))


  protein <<- read.table(paste(scratch, 'proteinGroups.txt', sep='/'),
                        sep="\t",
                        stringsAsFactors = F,
                        header=T)


  grp2 <<- QCProteinReport(paste("workunit:", yaml.config$job_configuration$workunit_id) , maxNA=3, nrPeptides=2)
